...
def handle_new_job(jobId,logger, lock):
    ...
    while not wait_finish_job(current_namespace, f"{body['metadata']['name']}-configure-job",logger):
    ...
    if OperatorConfig.DEBUG_NO_CLEANUP is None:
        try:
            ...
            api.delete_namespaced_job(namespace=current_namespace, name=name, propagation_policy='Foreground',grace_period_seconds=1)
    ...
    if sqlstatus==30:
        ...
        # Wait configure pod to finish
        while not wait_finish_job(current_namespace, f"{body['metadata']['name']}-algorithm-job",logger):
            ...
            if shouldstop is True:
                stop_specific_job(current_namespace,body['metadata']['name']+"-algorithm-job",logger)
                break
            time.sleep(5.0)
    else:
    ...
    if OperatorConfig.FILTERING_CONTAINER:
        ...
        while not wait_finish_job(current_namespace, f"{body['metadata']['name']}-filter-job",logger):
    ...
    # Wait configure pod to finish
    while not wait_finish_job(current_namespace, f"{body['metadata']['name']}-publish-job",logger):
    ...
    while True:
        ...
        for job in jobs:
            ...
            if current_jobs>=OperatorConfig.ENVIROMENT_maxJobs:
                ...
                break
            thread=Thread(target=handle_new_job, args=(job,logger,lock,))
            thread.start()
        time.sleep(5)
